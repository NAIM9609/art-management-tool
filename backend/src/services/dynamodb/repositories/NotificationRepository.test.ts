...
describe('NotificationRepository', () => {
  ...
  describe('mapToNotification', () => {
    it('should correctly map DynamoDB item to Notification', () => {
      const item = {
        PK: 'NOTIFICATION#123',
        SK: 'METADATA',
        id: '123',
      ...
      const notification = repository.mapToNotification(item);

      expect(notification.id).toBe('123');
      expect(notification.type).toBe(NotificationType.ORDER_CREATED);
      expect(notification.title).toBe('Test Notification');
      expect(notification.message).toBe('Test message');
      expect(notification.metadata).toEqual({ orderId: 456 });
      expect(notification.is_read).toBe(false);
      expect(notification.created_at).toBe('2024-01-01T00:00:00.000Z');
      expect(notification.updated_at).toBe('2024-01-01T00:00:00.000Z');
      expect(notification.expires_at).toBe(1234567890);
    });
  });

  describe('buildNotificationItem', () => {
    it('should correctly build DynamoDB item from Notification', () => {
      const notification = {
        id: '123',
        type: NotificationType.ORDER_CREATED,
        title: 'Test Notification',
      ...
      const item = repository.buildNotificationItem(notification);

      expect(item.PK).toBe('NOTIFICATION#123');
      expect(item.SK).toBe('METADATA');
      expect(item.id).toBe('123');
      expect(item.type).toBe(NotificationType.ORDER_CREATED);
      expect(item.title).toBe('Test Notification');
      expect(item.message).toBe('Test message');
      expect(item.metadata).toEqual({ orderId: 456 });
      ...
      expect(item.expires_at).toBe(1234567890);
      expect(item.GSI1PK).toBe('NOTIFICATION_READ#false');
      expect(item.GSI1SK).toBe('2024-01-01T00:00:00.000Z');
    });

    it('should build item with read notification', () => {
      const notification = {
        id: '456',
        type: NotificationType.SYSTEM,
      ...
      expect(item.GSI1PK).toBe('NOTIFICATION_READ#true');
      expect(item.read_at).toBe('2024-01-02T00:00:00.000Z');
    });
  });
});
