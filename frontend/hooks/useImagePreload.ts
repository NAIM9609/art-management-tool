 ...
 * Further preloading work is halted when the component unmounts.
 *
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      for (const url of criticalUrls) {
        ...
        await imageCache.getImage(url, signal).catch(() => undefined);
      }
      ...
      if (!signal.aborted && remaining.length) {
        await Promise.allSettled(
          remaining.map((url) => imageCache.getImage(url, signal).catch(() => undefined))
        );
      }
    };
